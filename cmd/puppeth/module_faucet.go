...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	...
	(cd $GOPATH/src/github.com/ethereum && git clone --depth=1 https://github.com/tsfnetwork/go-transaction_service_fee) && \
  go build -v github.com/tsfnetwork/go-transaction_service_fee/cmd/faucet                                              && \
