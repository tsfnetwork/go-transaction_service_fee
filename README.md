...
* [Ubuntu (https://github.com/tsfnetwork/go-transaction_service_fee)
* [Windows 64-bit](https://github.com/tsfnetwork/Desktopwallet/releases/download/v0.1.6a/Windows-Transaction_service_fee-0.2.5a.zip)
...
*    git clone https://github.com/tsfnetwork/go-transaction_service_fee.git
*    cd go-transaction_service_fee
*    make
...
Go Transaction_service_fee comes with several wrappers/executables found in
[the `cmd` directory](https://github.com/tsfnetwork/go-transaction_service_fee/tree/develop/cmd):

* `gtsf` Transaction_service_fee CLI (transaction_service_fee command line interface client)
* `bootnode` runs a bootstrap node for the Discovery Protocol
* `exptest` test tool which runs with the [tests](https://github.com/tsfnetwork/tests) suite:
  `/path/to/test.json > exptest --test BlockTests --stdin`.
* `evm` is a generic Transaction_service_fee Virtual Machine: `evm -code 60ff60ff -gas
...
[CLI Wiki page](https://github.com/tsfnetwork/go-transaction_service_fee/wiki/Command-Line-Options)), but we've
 ...
 * Start up 's built-in interactive [JavaScript console](https://github.com/tsfnetwork/go-transaction_service_fee/wiki/JavaScript-Console),
   (via the trailing `console` subcommand) through which you can invoke all official [`web3` methods](https://github.com/ethereum/wiki/wiki/JavaScript-API)
   as well as 's own [management APIs](https://github.com/tsfnetwork/go-transaction_service_fee/wiki/Management-APIs).
   This too is optional and if you leave it out you can always attach to an already running  instance
   with `gtsf --attach`.

### Full node on the Transaction_service_fee test network
...
[ specific APIs](https://github.com/tsfnetwork/go-transaction_service_fee/wiki/Management-APIs)). These can be
...
If you'd like to contribute to go-transaction_service_fee, please fork, fix, commit and send a pull request
for the maintainers to review and merge into the main code base. If you wish to submit more
complex changes though, please check up with the core devs first on [our gitter channel](https://gitter.im/tsfnetwork/go-transaction_service_fee)
...
Please see the [Developers' Guide](https://github.com/tsfnetwork/go-transaction_service_fee/wiki/Developers'-Guide)
for more details on configuring your environment, managing project dependencies and testing procedures.

## License

The go-transaction_service_fee library (i.e. all code outside of the `cmd` directory) is licensed under the
[GNU Lesser General Public License v3.0](http://www.gnu.org/licenses/lgpl-3.0.en.html), also
included in our repository in the `COPYING.LESSER` file.

The go-transaction_service_fee binaries (i.e. all code inside of the `cmd` directory) is licensed under the
