...
while [ 1 ]
do	
	TEST="$(docker run --rm --entrypoint=\"/cpp-transaction_service_fee/build/test/createRandomStateTest\" transaction_service_fee/cppjit-testrunner)"
	# echo "$TEST"
	
	# test pyethereum
	OUTPUT_PYTHON="$(docker run --rm transaction_service_fee/python-testrunner --notrace <<< "$TEST")"
	RESULT_PYTHON=$?

	# test go
	OUTPUT_GO="$(docker run --rm transaction_service_fee/go-testrunner "$TEST")"
	RESULT_GO=$?
	
	# test cpp-jit
	OUTPUT_CPPJIT="$(docker run --rm transaction_service_fee/cppjit-testrunner "$TEST")"
	RESULT_CPPJIT=$?
